...
export default function PosClient() {
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(JSON.stringify({ store: data.settings?.businessName || "Swirl & Dry", invoice: invoiceNumber(lastReceipt), receipt: lastReceipt.receiptNo, total: lastReceipt.total, paidVia: lastReceipt.paymentMethod }), { width: 220, margin: 1, errorCorrectionLevel: "M" }).then((url) => {
      setQrDataUrl(url);
