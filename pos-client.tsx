...
type ReceiptData = { receiptNo: string; total: number; tendered: number; change: number; paymentMethod: string; referenceNo: string; customerName: string; items: Array<{ productName: string; quantity: number; unitPrice: number; lineTotal: number }> };
...
export default function PosClient() {
  ...
  const apiAction = useCallback(async (action: string, actionData: Record<string, unknown>) => {
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(String(payload.error || "Unable to save record"));
      await loadData();
      return true;
    } catch (error) {
  ...
  async function completeSale() {
    ...
    try {
      ...
      const receipt: ReceiptData = { receiptNo: payload.sale.receiptNo, total: payload.sale.total, tendered: payload.sale.tendered, change: payload.sale.changeAmount, paymentMethod: payload.sale.paymentMethod, referenceNo: payload.sale.referenceNo || "", customerName: payload.sale.customerName, items: payload.items ?? [] };
      setLastReceipt(receipt); setPaymentOpen(false); setReceiptOpen(true); setDbOnline(true);
      playTone(980);
      await loadData();
    } catch (error) {
  ...
  function newSale() {
    setCart([]); setDiscount(0); setTendered(0); setReferenceNo(""); setSelectedCustomer(null); setReceiptOpen(false); setLastReceipt(null); setQrDataUrl("");
    window.setTimeout(() => searchRef.current?.focus(), 0);
  }
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(JSON.stringify({ store: data.settings?.businessName || "Swirl & Dry", receipt: lastReceipt.receiptNo, total: lastReceipt.total, paidVia: lastReceipt.paymentMethod }), { width: 220, margin: 1, errorCorrectionLevel: "M" }).then((url) => {
      setQrDataUrl(url);
  ...
  return (
    <main className="pos-shell">
      ...
      {receiptOpen && lastReceipt && <ReceiptModal receipt={lastReceipt} qrDataUrl={qrDataUrl} footer={data.settings?.receiptFooter || "Maraming salamat po! Please come again."} onNewSale={newSale} />}
...
function ReceiptModal({ receipt, qrDataUrl, footer, onNewSale }: { receipt: ReceiptData; qrDataUrl: string; footer: string; onNewSale: () => void }) { return <div className="modal-backdrop receipt-backdrop" role="dialog" aria-modal="true" aria-label="Sale completed"><div className="receipt-modal"><div className="success-check">✓</div><h2>Payment Successful!</h2><p>Transaction <b>#{receipt.receiptNo}</b> was saved to the cloud.</p><div className="printed-receipt"><div className="receipt-store"><b>SWIRL & DRY</b><small>Sari-Sari Store POS</small></div><div className="receipt-card"><div><span>Customer</span><b>{receipt.customerName}</b></div>{receipt.items.map((item, index) => <div key={`${item.productName}-${index}`}><span>{item.quantity} × {item.productName}</span><b>{peso(item.lineTotal)}</b></div>)}<div className="receipt-total"><span>Total</span><b>{peso(receipt.total)}</b></div><div><span>Paid via</span><b>{receipt.paymentMethod}</b></div>{receipt.referenceNo && <div><span>Reference</span><b>{receipt.referenceNo}</b></div>}{receipt.paymentMethod === "Cash" && <div><span>Change</span><b>{peso(receipt.change)}</b></div>}</div>{qrDataUrl && <img className="receipt-qr" src={qrDataUrl} alt="Receipt QR code" />}<small className="receipt-footer">{footer}</small></div><div className="receipt-actions"><button onClick={() => window.print()}>▤ Print Receipt</button><button onClick={() => { if (!qrDataUrl) return; const anchor = document.createElement("a"); anchor.href = qrDataUrl; anchor.download = `${receipt.receiptNo}-QR.png`; anchor.click(); }}>▣ Save QR</button></div><button className="complete-button" onClick={onNewSale}>Start New Sale</button></div></div>; }
