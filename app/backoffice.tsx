...
function Settings({ online, products, data, onAction }: { online: boolean; products: ProductRecord[]; data: PosData; onAction: Props["onAction"] }) {
  ...
  const [businessName, setBusinessName] = useState(source?.businessName || "ATE ANNA'S STORE POS");
  ...
  function backup() {
    ...
    const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `ate-annas-store-pos-backup-${todayText()}.json`; anchor.click(); URL.revokeObjectURL(url);
  }
...
function payslipHtml(employee: EmployeeRecord) {
  ...
  return `<article><h2>ATE ANNA'S STORE POS</h2><p>Employee Payslip</p><hr><h3>${employee.fullName}</h3><small>${employee.role}</small><dl><div><dt>Work Days</dt><dd>${employee.workDays}</dd></div><div><dt>Rate / Day</dt><dd>${peso(employee.dailyRate)}</dd></div><div><dt>Gross Salary</dt><dd>${peso(gross)}</dd></div><div><dt>Store Credit</dt><dd>−${peso(employee.storeCredit)}</dd></div><div class="net"><dt>NET SALARY</dt><dd>${peso(net)}</dd></div></dl><footer>Employee Signature: ____________________</footer></article>`;
}
